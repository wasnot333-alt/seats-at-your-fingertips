...
interface MultiBookingTicketProps {
  bookings: Booking[];
}
...
export default function MultiBookingTicket({ bookings }: MultiBookingTicketProps) {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>Meditation Seat Ticket</title>
          <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .level-seat { display: flex; justify-content: center; gap: 16px; flex-wrap: wrap; margin: 16px 0; }
            .level-item { background: #fff; padding: 12px 20px; border-radius: 12px; border: 1px solid #e5e5e5; text-align: center; }
            .level-name { font-size: 12px; color: #7c3aed; font-weight: 600; margin-bottom: 4px; }
            .seat-number { font-size: 24px; font-weight: bold; color: #92400e; }
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        {/* Multiple Level Seats */}
        <div className="seat-section text-center my-6">
          <p className="text-xs text-muted-foreground uppercase tracking-widest mb-4">Your Reserved Seats</p>
          <div className="level-seat flex justify-center gap-4 flex-wrap">
            {bookings.map((booking, index) => (
              <div 
                key={booking.id || index} 
                className="level-item bg-background/50 px-5 py-3 rounded-xl border border-border text-center"
              >
                <p className="level-name text-xs font-semibold text-purple-600 flex items-center justify-center gap-1 mb-1">
                  <Zap className="w-3 h-3" />
                  {booking.sessionLevel || 'Level 1'}
                </p>
                <p className="seat-number text-2xl font-display font-bold text-amber-800">
        ...
        <div className="details space-y-3 mb-6">
          <div className="detail-row flex justify-between items-center py-2 border-b border-border/50">
            ...
            <span className="detail-value text-sm font-bold text-amber-900">
              {displayValue(firstBooking.customerName)}
            </span>
          </div>
          <div className="detail-row flex justify-between items-center py-2 border-b border-border/50">
            <span className="detail-label text-xs text-muted-foreground flex items-center gap-2">
            ...
            <span className="detail-value text-sm font-mono font-bold text-amber-900">
              {displayValue(firstBooking.codeUsed)}
            </span>
          </div>
          <div className="detail-row flex justify-between items-center py-2 border-b border-border/50">
            <span className="detail-label text-xs text-muted-foreground flex items-center gap-2">
            ...
            <span className="detail-value text-sm font-bold text-amber-900">
              {formatBookingTime(firstBooking.bookingTime)}
            </span>
          </div>
          <div className="detail-row flex justify-between items-center py-2">
            <span className="detail-label text-xs text-muted-foreground flex items-center gap-2">
              <Armchair className="w-4 h-4" /> Levels Booked
            </span>
            <span className="detail-value text-sm font-bold text-amber-900">
              {bookings.length} {bookings.length === 1 ? 'Level' : 'Levels'}
            </span>
          </div>
        </div>
